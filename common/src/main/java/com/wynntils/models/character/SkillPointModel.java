    ...
    private List<ClickAction> calculateClickActions(int[] targetSkillPoints) {
        ...
        for (Map.Entry<Skill, Integer> entry : negatives.entrySet()) {
            ...
            for (int i = 0; i < fives; i++) {
                actions.add(new ClickAction(slot, SDLMouse.SDL_BUTTON_RIGHT, true));
            ...
            for (int i = 0; i < ones; i++) {
                actions.add(new ClickAction(slot, SDLMouse.SDL_BUTTON_RIGHT, false));
                if (!confirmationCompleted) {
                    actions.add(new ClickAction(slot, SDLMouse.SDL_BUTTON_RIGHT, false));
                    confirmationCompleted = true;
                }
        ...
        for (Map.Entry<Skill, Integer> entry : positives.entrySet()) {
            ...
            for (int i = 0; i < fives; i++) {
                actions.add(new ClickAction(slot, SDLMouse.SDL_BUTTON_LEFT, true));
            }
            for (int i = 0; i < ones; i++) {
                actions.add(new ClickAction(slot, SDLMouse.SDL_BUTTON_LEFT, false));
            }
        }
