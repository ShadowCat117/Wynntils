...
public final class WynntilsBookSettingsScreen extends WynntilsScreen implements TextboxScreen {
    ...
    private void importSettings(int clicked) {
        if (clicked == 0) {
            McUtils.sendMessageToClient(Component.literal("Importing all settings"));
        } else if (clicked == 1) {
            McUtils.sendMessageToClient(Component.literal("Importing settings for selected config"));
        }
    }
    ...
    private void exportSettings(int clicked) {
        if (clicked == 0) {
            McUtils.sendMessageToClient(Component.literal("Exporting all settings"));
        } else if (clicked == 1) {
            McUtils.sendMessageToClient(Component.literal("Exporting settings for selected config"));
        }
    }
}
