...
public class ConfigProfileScreen extends WynntilsScreen {
    ...
    public boolean keyPressed(KeyEvent event) {
        if (McUtils.options().keyLeft.matches(event) || event.key() == SDLScancode.SDL_SCANCODE_LEFT) {
            updateFocusedProfile(-1);
        } else if (McUtils.options().keyRight.matches(event) || event.key() == SDLScancode.SDL_SCANCODE_RIGHT) {
            updateFocusedProfile(1);
        } else if (event.key() == SDLScancode.SDL_SCANCODE_SPACE || event.key() == SDLScancode.SDL_SCANCODE_RETURN) {
