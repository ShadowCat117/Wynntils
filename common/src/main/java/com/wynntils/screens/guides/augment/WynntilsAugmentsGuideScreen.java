...
public final class WynntilsAugmentsGuideScreen
        extends WynntilsListScreen<AugmentItemStack, GuideAugmentItemStackButton> {
    private static final int ELEMENTS_COLUMNS = 7;
    private static final int ELEMENT_ROWS = 7;

    private final List<AugmentItemStack> parsedItemCache = new ArrayList<>();

    private WynntilsAugmentsGuideScreen() {
        super(Component.translatable("screens.wynntils.wynntilsGuides.augments.name"));
    }

    public static Screen create() {
        return new WynntilsAugmentsGuideScreen();
    }
    ...
    protected void doInit() {
        if (parsedItemCache.isEmpty()) {
            for (AmplifierInfo amplifierInfo : Models.Rewards.getAllAmplifierInfo()) {
                parsedItemCache.add(new AmplifierItemStack(amplifierInfo));
            }

            parsedItemCache.add(new SimulatorItemStack());
            parsedItemCache.add(new InsulatorItemStack());
        }
    ...
    public void doExtractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        renderTitle(guiGraphics, I18n.get("screens.wynntils.wynntilsGuides.augments.name"));
    ...
    @Override
    protected GuideAugmentItemStackButton getButtonFromElement(int i) {
        int xOffset = (i % ELEMENTS_COLUMNS) * 20;
        int yOffset = ((i % getElementsPerPage()) / ELEMENTS_COLUMNS) * 20;

        return new GuideAugmentItemStackButton(
